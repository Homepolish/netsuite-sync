 ...
 * NetSuite WSDL is at https://webservices.netsuite.com/wsdl/v2018_1_0/netsuite.wsdl as of this writing
 */
var crypto = require('crypto')
var moment = require('moment')
var Nonce = require('nonce-fast')
...
function makeSoapMsg(bodyTemplateFilename, data) {
    ...
    var requestData = _.extend(generateTokenConfig(nsConf), { soapBody: _.template(bodyContent)(data) });
    debug('requestData', requestData)
    var msg = _.template(headerContent)(requestData)
    // fs.writeFileSync('output.xml', msg, 'utf-8');
    return msg;
}

function computeHash(baseString, key, algorithm = 'sha256') {
    return crypto.createHmac(algorithm, key).update(baseString).digest('base64')
}

function getNonce() { return Nonce(15)() }

function getTimestamp() { return Number.parseInt(moment().valueOf() / 1000) }

function generateTokenConfig(nsConf) {
    const nonce = getNonce()
    const timestamp = getTimestamp()
    const { account, consumerKey, token, consumerSecret, tokenSecret } = nsConf
    const baseString = `${account}&${consumerKey}&${token}&${nonce}&${timestamp}`
    ...
    return {
        ...
        timestamp,
        nonce,
        signature: computeHash(baseString, key)
    }
}
